...
- **插件自动安装(activation)**:新增 `home.activation.installHerdrPalettePlugin`(`entryAfter writeBoundary`):已装则跳过(`herdr plugin list | grep ramarivera.palette`),未装则 `herdr plugin install ramarivera/herdr-palette --yes`。实测 `herdr plugin install` 无需 server、`plugin list` 无需 server 输出含 `ramarivera.palette` → 幂等 guard 可靠。需 cargo(ide profile 的 rust 模块已带)+ 联网;首次 switch cargo 编译(分钟级),之后命中跳过瞬时;失败不阻断 switch(`|| true`)。
