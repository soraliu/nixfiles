...
- **插件自动安装(activation)**:`home.activation.installHerdrPalettePlugin`(`entryAfter writeBoundary`)。幂等判定基于「二进制是否已构建」(检查 `~/.config/herdr/plugins/github/ramarivera.palette-*/target/release/herdr-palette`)而非「插件是否已链入」——这样能自愈上一版的一个坏状态:旧 guard 用 `herdr plugin list | grep ramarivera.palette` 判“已装”,但若安装被 trust prompt 打断 / cargo build 未完成,manifest 仍会链上、二进制却缺失 → 之后 `alt+p` 报 `Unable to spawn target/release/herdr-palette ... No viable candidates found in PATH`;新版二进制缺失即自动重装(重克隆 + 重建),下次 switch 自愈。安装用 `--yes </dev/null`:把 stdin 置为非 tty → herdr 走 `--yes` 路径,**不再弹 `Install this plugin? [y/N]`**(随 herdr 版本“stdin 交互时 `--yes` 不抑制提示”，故需主动强制非交互)。需 cargo(ide profile 的 rust 模块已带)+ 联网;首次 switch cargo 编译(分钟级),之后命中跳过近乎瞬时;失败不阻断 switch(`|| true`)。
