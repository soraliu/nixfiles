...
- **插件安装与暴露二进制(activation)**:`home.activation.installHerdrPalettePlugin`(`entryAfter writeBoundary`)三步:
  1. 构建二进制:`herdr plugin install ramarivera/herdr-palette --yes </dev/null`(缺才重装;`</dev/null` 强制非交互 stdin → `--yes` 生效,不再弹 `Install this plugin? [y/N]`)。幂等判基于「二进制是否已构建」(`~/.config/herdr/plugins/github/ramarivera.palette-*/target/release/herdr-palette`)而非「插件是否已链入」——这能自愈此前「被 trust prompt 打断 / cargo build 未完成、manifest 已链但二进制缺失」的坏状。
  2. 链接 cargo 子 manifest:`herdr plugin link <plugin_root>/cargo` 覆盖 herdr 默认注册的「根 manifest」。**根因**：herdr 0.7.5 的 GitHub 流程 `plugin install` 注册的「根 manifest」pane 命令是相对路径 `target/release/herdr-palette`,而 pane 由 herdr 服务器用「服务器自己的 PATH」spawn(`plugin pane open` 实测)——相对路径文件名含 `/`,PATH 搜索永远找不到,`alt+p` 会报 `Unable to spawn target/release/herdr-palette ... No viable candidates found in PATH`;cargo 子 manifest 的 pane 命令是「裸」`herdr-palette`(靠 PATH 找),重叠制后才可行。
  3. 软链二进制到交互 PATH:`ln -sfn <binary> ~/.local/bin/herdr-palette`。`~/.local/bin` 已在交互 / 服务器 PATH(由 `home/modules/sys/shell/default.nix` 注入)、且未被 HM 内容托管(不会被 reconcile 清掌)→ herdr 服务器 spawn 时能找到 `herdr-palette`。
  - 自愈旧坏状(根 manifest 注册、spawn 报 PATH)：merge 一次 `switch-darwin soraliu` 即修复(2/3 步每次 switch 刷新);`herdr-palette` 不在 nixpkgs(stable/unstable 均无)放暂时只能走 cargo 构建 + 软链。
  - 需 cargo(ide profile 的 rust 模块已带)+ 联网;首次构建分钟级,之后瞬时;失败不阻断 switch(`|| true`)。交互验证:`alt+p` 应打开 Palette 覆盖层;手动排查见 `home/modules/ide/herdr/default.nix` 注释。
